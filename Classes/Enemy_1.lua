...
function Enemy_1.new(distance, shadow, speed)
    local self = setmetatable({}, Enemy_1)
    self.x = distance
    self.y = 312
    self.width = 100
    self.height = 160
    self.animation1 = love.graphics.newImage("Characters/GoblinOccultistIdle1.png")
    self.animation2 = love.graphics.newImage("Characters/GoblinOccultistIdle2.png")
    self.animation3 = love.graphics.newImage("Characters/GoblinOccultistIdle3.png")
    self.animation4 = love.graphics.newImage("Characters/GoblinOccultistIdle4.png")
...
function Enemy_1:update(dt)
    ...
    if self.animationTimer >= self.animationDelay then
        ...
        elseif self.currentAnimation == self.animation3 then
            self.currentAnimation = self.animation4
        elseif self.currentAnimation == self.animation4 then
            self.currentAnimation = self.animation1
        end
